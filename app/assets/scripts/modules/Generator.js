...
I just told you! You've killed me! Fry! Quit doing the right thing, you jerk! Michelle, I don't regret this, but I both rue and lament it. Morbo can't understand his teleprompter because he forgot how you say that letter that's shaped like a man wearing a hat.`,
    ...
    this.form = document.querySelector(".lorem-form");
    this.amount = document.getElementById("amount");
    this.result = document.querySelector(".lorem-text");
    this.events();
  }
  events() {
    this.form.addEventListener("submit", (e) => {
      e.preventDefault();
      this.value = parseInt(this.amount.value);
      this.random = Math.floor(Math.random() * this.text.length);
      if (isNaN(this.value) || this.value < 0 || this.value > 9) {
        this.result.innerHTML = `<p class="result">${
          this.text[this.random]
        }</p>`;
      } else {
        let tempText = this.text.slice(0, this.value);
        tempText = tempText
          .map((item) => {
            return `<p class="result">${item}</p>`;
          })
          .join("");
        this.result.innerHTML = tempText;
      }
    });
  }
}
